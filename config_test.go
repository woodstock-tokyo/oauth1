...
func TestConfigAccessToken(t *testing.T) {
	...
	accessToken, accessSecret, err := config.AccessToken("request_token", "request_secret", expectedVerifier, nil)
	assert.Nil(t, err)
...
func TestConfigAccessToken_InvalidAccessTokenURL(t *testing.T) {
	...
	accessToken, accessSecret, err := config.AccessToken("any_token", "any_secret", "any_verifier", nil)
	assert.NotNil(t, err)
